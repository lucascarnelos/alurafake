...
public class CreateSingleChoiceTaskUseCaseImpl implements CreateSingleChoiceTaskUseCase {
    ...
    public CreateSingleChoiceTaskUseCaseImpl(TaskPersistenceGateway taskPersistenceGateway, CoursePersistenceGateway coursePersistenceGateway, ValidaTaskUseCase validaTaskUseCase, OrderingTasksUseCase orderingTasksUseCase) {
    ...
    private void valid(Task task){
        ...
        boolean alternativaTrueUnicaError = false;
        ...
        if(taskOptions != null && !taskOptions.isEmpty()){
            for(TaskOption taskOption : taskOptions){
                if(taskOption.getCorrect())
                    correctOptions++;
                if(correctOptions > 1)
                    alternativaTrueUnicaError = true;
        ...
        if(maximoAlternativasTask) errors.add(new ErrorItem("options","A atividade deve ter no minimo 2 e no máximo 5 alternativas."));
        if(alternativaTrueUnicaError) errors.add(new ErrorItem("isCorrect","A atividade deve ter uma única alternativa correta."));
